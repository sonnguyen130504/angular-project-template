...
- **Proportional Ratios:** Always maintain the ratio and proportions of components (from internal detail to global layout) intact across all screens for consistent UI/UX.
- **Scroll-Mode Minimization:** Restrict switching layout containers to scroll mode as much as possible. If layout overflow is minimal, reduce component sizes (paddings, margins, fonts) or redesign rather than displaying scrollbars. Only design for scroll if overflow is substantial.
- **Grid Blowout Prevention:** In responsive CSS Grids on mobile (e.g., `grid-template-columns: 1fr`), always apply `min-width: 0` to direct grid children. Without this, a grid item's `min-content` size (e.g., a wide table, a long `<pre><code>` block, or a nested Kanban board) will forcefully expand the parent grid column, causing catastrophic horizontal overflow on the entire page.
- **Bulletproof Horizontal Scrolling:** When building horizontally scrollable panes (like Kanban boards or Tabs) on mobile, avoid using CSS Grid for the scroll container. Use Flexbox (`display: flex; flex-wrap: nowrap; overflow-x: auto;`) combined with `flex-shrink: 0` and a fixed/min `width` on the children. This guarantees reliable touch-scrolling behavior.
- **Long String Wrapping:** In narrow mobile views, exceptionally long unbroken strings (like server error logs, URLs, or data hashes) must be explicitly tamed using `word-break: break-all` and `white-space: pre-wrap`. Never let a long text string push an app shell beyond `100vw`.
- **The Strict-Footprint Rule**: When building galleries of system states (e.g. empty/error/offline), force a strictly identical `width` and `height` container (e.g., `450px`). The outer UI must feel like a rock-solid, unchanging canvas where only the internal content swaps. Do not let containers jitter or adapt their height to the content.
- **The Smart-Swapping Rule**: Never lazily stack dynamic elements (like a Success alert) on top of old content (like an Error log) if it causes the container to overflow or trigger a scrollbar. Intelligently hide/swap out the non-critical old elements to make room. Avoid triggering scroll mode unless absolutely necessary.
- **The Absolute-Alignment Rule**: Ensure universal alignment for key interactive elements across different states. Do not rely on container-level centering which shifts buttons vertically. Use `margin-top: auto` or similar techniques to push action buttons to a strict bottom baseline.
